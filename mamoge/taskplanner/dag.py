...
def G_routemap_fully_connected(nodes):
    ...
    for (i, ni), (j, nj) in itertools.combinations(nodes, 2):
        # print("ij", i, j, ni, nj)
        G.add_node(i, name=f'{i}', layer=1,
                   location=GPSLocation(latitude=ni["latitude"],
                                        longitude=ni["longitude"],
                                        altitude=ni["altitude"]))
        G.add_node(j, name=f'{j}', layer=1,
                   location=GPSLocation(latitude=nj["latitude"],
                                        longitude=nj["longitude"],
                                        altitude=ni["altitude"]))
        G.add_edge(i, j)
...
def DAG_all_parallel(G_routemap, base_id, nodes):
    ...
    for n_id, n in nodes:
        ...
        task_id = f"{n_id}"
        G_tasks.add_node(task_id, name=task_id, layer=1,
    ...
    G_tasks.add_node(end_id, name="end", layer=2,
                     location=NXLayerLocation(layer_id=end_id,
                                              base_id=base_id,
                                              G_layer=G_tasks,
                                              G_base=G_routemap,
                                              name=f"{base_id}"))
    [G_tasks.add_edge(task_id, end_id)
     for task_id in list(G_tasks.nodes)[1:-1]]
